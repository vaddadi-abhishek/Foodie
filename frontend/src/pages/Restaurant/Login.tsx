...
export default function RestaurantLogin() {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      // TODO: Redirect to a restaurant-specific dashboard, e.g., /restaurant/dashboard
      navigate("/");
    }
  ...
  const onSubmit = async (data: LoginFormData) => {
    try {
      // TODO: Update with the correct restaurant login API endpoint
      const response = await fetch("http://127.0.0.1:8000/api/restaurants/login/", {
        method: "POST",
      ...
      // TODO: Redirect to a restaurant-specific dashboard
      navigate("/");
    } catch (error) {
      alert(error);
    }
